...
- the generic post list currently returns summary fields only and does not inline cover URLs
- Android may still call `GET /api/posts/{postId}` to resolve a stable nested media cover when needed
  ...
  "mediaItems": [
    {
      ...
      "media": {
        ...
        "thumbnailUrl": "/api/media/files/media_001?variant=preview",
...
- Android album pages still primarily consume `GET /api/albums/{albumId}/posts`
- `GET /api/posts` is now available for REAL Repository and generic post listing use cases
- `POST /api/posts`, `POST /api/posts/{postId}/media`, `PATCH /api/posts/{postId}/media-order`, and `PATCH /api/posts/{postId}/cover` should all trigger Android-side refresh of feed / album / detail state
